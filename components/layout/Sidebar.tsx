...
export default function Sidebar({ isOpen = false, onClose }: SidebarProps) {
  ...
  return (
    <aside
      className={`fixed left-0 top-0 bottom-0 w-64 bg-[#111] border-r border-white/[0.07] flex flex-col z-50 transition-transform duration-300 ${
    ...
    >
      ...
      <div className="px-6 py-5 border-b border-white/[0.07] flex items-center justify-between">
        ...
        {onClose && (
          <button
            onClick={onClose}
            className="lg:hidden text-white/40 hover:text-white p-1 rounded-md transition-colors"
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              onClick={onClose}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-mono transition-all ${isActive
