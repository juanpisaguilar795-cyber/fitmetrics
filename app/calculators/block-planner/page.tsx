...
export default function BlockPlannerPage() {
  ...
  return (
    <div className="p-8 max-w-5xl">
                ...
                return (
                  <tr
                    ...
                    className={`border-b border-white/[0.03] hover:bg-white/[0.02] cursor-pointer transition-colors ${
                      isCurrent ? 'bg-[#C8FF00]/[0.03] text-[#C8FF00]' : ''
                    }`}
                  >
                ...
                {DEADLIFT_PROGRESSION.map((w) => {
                  ...
                  return (
                    <tr
                      ...
                      onClick={() => setSelectedWeek(w.week)}
                      className={`border-b border-white/[0.03] hover:bg-white/[0.02] cursor-pointer transition-colors ${
                        isCurrent ? 'bg-orange-500/[0.03] text-orange-400' : ''
                      }`}
                    >
                      <td className="px-5 py-2 text-xs font-mono font-bold">{w.week}</td>
                      <td className="px-5 py-2 text-xs font-mono text-white/40">{(w.pctMin * 100).toFixed(1)}% {w.pctMin !== w.pctMax ? `- ${(w.pctMax * 100).toFixed(1)}%` : ''}</td>
                ...
                {SQUAT_PROGRESSION.map((w) => {
                  ...
                  return (
                    <tr
                      ...
                      onClick={() => setSelectedWeek(w.week)}
                      className={`border-b border-white/[0.03] hover:bg-white/[0.02] cursor-pointer transition-colors ${
                        isCurrent ? 'bg-blue-500/[0.03] text-blue-400' : ''
                      }`}
                    >
                      <td className="px-5 py-2 text-xs font-mono font-bold">{w.week}</td>
                      <td className="px-5 py-2 text-xs font-mono text-white/40">{(w.pctMin * 100).toFixed(1)}% {w.pctMin !== w.pctMax ? `- ${(w.pctMax * 100).toFixed(1)}%` : ''}</td>
      ...
      <div className="flex border-b border-white/[0.07] mb-6 gap-2">
        {([
          ...
          <button
            ...
            className={`flex-1 pb-3 text-center transition-all focus:outline-none border-b-2 ${
              activeTab === tab.key
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4 my-6">
              <div className="bg-[#181818] p-5 rounded-lg border border-white/[0.04]">
                ...
                <p className="font-black text-5xl text-[#C8FF00]">
                  {benchHeavyWeight}
                </p>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4 my-6">
              <div className="bg-[#181818] p-5 rounded-lg border border-white/[0.04]">
                ...
                <p className="font-black text-5xl text-green-400">
      ...
      <div className="bg-[#111] border border-white/[0.07] rounded-xl p-6">
        ...
        <div className="grid grid-cols-2 gap-6 my-6 border-b border-white/[0.04] pb-6">
                ...
                {[
                  ...
                  <button
                    ...
                    className={`py-2 text-xs font-mono uppercase tracking-widest rounded-lg border transition-all ${
                      questionnaire.completedAll === ans.key
                        ? 'bg-[#C8FF00]/10 border-[#C8FF00]/30 text-[#C8FF00]'
                        : 'bg-[#181818] border-white/[0.07] text-white/40 hover:text-white/60'
                    }`}
                  >
            ...
            <div className="grid grid-cols-4 gap-2">
              {[
                ...
                <button
                  ...
                  className={`py-3 text-xs font-mono rounded-lg border transition-all flex flex-col items-center justify-center disabled:opacity-30 ${
                    questionnaire.rir === r.key && questionnaire.completedAll === 'yes'
        ...
        <div className={`border rounded-xl p-5 ${verdict.color}`}>
          <p className="font-black text-3xl uppercase tracking-tight mb-2">{verdict.action}</p>
